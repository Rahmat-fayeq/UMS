...
{
  $id=$_GET['nid'];
  $getN=$connect->query("SELECT *FROM news WHERE id=$id");
  $n=$getN->fetch_object();
}
...
if(isset($_POST['edit']))
{
  ...
  $addnews=$connect->query("
    UPDATE news SET title='$title',image='$imagePath',body='$news',post_date='$date' WHERE id=$id");
 if($addnews)
 {
    move_uploaded_file($imageTmp, $imagePath);
    echo "<script>alert('News Update Successed');</script>";
 }
 else
 {
  echo "fail";
          ...
          <div class="box">
            ...
            <div class="box-body">
                  <form method="post" class="form-group" enctype="multipart/form-data" id="newForm">
                      ...
                      <input type="text" name="title" placeholder="New Title" class="form-control" data-bvalidator="required" value="<?php echo $n->title?>">
                      ...
                      <textarea class="form-control" rows="10" cols="20" placeholder="Wrtie The New Here!" data-bvalidator="required" name="news" ><?php echo $n->body;?></textarea>
